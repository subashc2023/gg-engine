...
| `cargo xtask ci --nightly` | Push tier + dist gate (incl. demo dist runs) + capability probe + aarch64-under-qemu determinism leg (WSL lane) + headless GPU tests and demo runs on pinned lavapipe + interactive suite (invisible, non-activating windows) |
