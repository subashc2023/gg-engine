...
| `cargo xtask dist` | Dist gate: build + run the exact `tier-dist` combination, prove the lab equipment unbolted and the input recorder present |
| `cargo xtask replay [--bless]` | Replay determinism material (§5.6): checks the curated replay's hash sequence against its baseline; `--bless` re-authors both, deliberately |
| `cargo xtask timers [--status\|--install\|--uninstall]` | The scheduled nightly/weekly tiers — idle-only, below-normal priority. Installing changes the machine, so nothing does it implicitly |
...
| `cargo run -p demo-02-mesh` | Demo 02 (M4A/M4B): a textured mesh under a fly camera whose state lives in the ECS. `--record f.ggrp` writes a replay, `--replay f.ggrp` plays it back, `--expect-hash <hex>` asserts it reproduced |
