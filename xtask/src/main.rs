...
fn usage() -> anyhow::Result<()> {
    anyhow::bail!(
        "usage: cargo xtask <command>\n\
         ...
         reload [--cross-tier|--segments|--chaos|--latency|--ui|--save|--editor|--launcher|--knob|--tetris|--menu|--platformer|--shooter|--orbit|--epoch|--node|--rules|--feel|--retune|--burn|--best|--settings|--progress|--keys|--window|--agent]  the gates that need the shell over a game dylib; no flag runs the set\n\
