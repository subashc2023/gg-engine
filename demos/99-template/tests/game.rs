...
impl Game {
    ...
    fn step(&mut self) {
        let ctx = TickCtx::detached(self.tick, 60, InputFrame::default(), InputFrame::default());
    ...
    /// The spinning cube specifically. The floor is a `Renderable` too, so
    /// `all::<Renderable>().first()` would name whichever archetype came first
    /// — a fact about `World`, not about this game.
    fn cube(&self) -> Renderable {
        let query = Query::<(&Spinner, &Renderable)>::new().unwrap();
        let mut found = None;
        self.world
            .each_ref(&query, |_, (_, shape): (&Spinner, &Renderable)| {
                found = Some(*shape);
            });
        found.expect("the cube")
    }
}

#[test]
fn one_tick_leaves_a_cube_a_floor_a_sun_and_an_eye() {
    let mut game = Game::load();
    game.step();
    assert_eq!(game.all::<Spinner>().len(), 1);
    assert_eq!(game.all::<Renderable>().len(), 2, "the cube and its floor");
...
fn bootstrap_run_again_spawns_nothing_new() {
    ...
    assert_eq!(game.all::<Spinner>().len(), 1);
    assert_eq!(game.all::<Renderable>().len(), 2);
    assert_eq!(game.all::<Light>().len(), 1);
    assert_eq!(game.all::<Eye>().len(), 1);
}

/// The cube rests *on* the floor rather than above it. The whole reason the
/// floor is here is that contact edge: it is where peter-panning shows, and a
/// gap of any size would hide it.
#[test]
fn the_cube_rests_on_the_floor() {
    let mut game = Game::load();
    game.step();
    let floor = game
        .all::<Renderable>()
        .into_iter()
        .find(|r| r.half_extent == FLOOR_HALF)
        .expect("a floor");
    // Approximately, and the tolerance is the point rather than slack: a
    // half-extent is `f32` (§4.1 — a size is never planetary), so widening two
    // of them and adding lands a few ULPs off a decimal that was never exact.
    let gap = floor.position.y + f64::from(FLOOR_HALF.y) + f64::from(CUBE_HALF);
    assert!(gap.abs() < 1e-6, "the cube floats {gap} m above the floor");
    // And the floor is a receiver, not a second caster: nothing spins it.
    assert_eq!(game.cube().half_extent, sim::Vec3::splat(CUBE_HALF));
}
