...
/// The music (§6 M43). Not fired by `fired[]` like the others — a loop is a
/// *level* rather than an event, so `step` writes it on the two edges that
/// change whether a game is in progress.
pub const CUE_MUSIC: u8 = 7;
/// How many there are.
...
pub fn voice_of(kind: u8) -> Sound {
    match kind {
        ...
        // A struck body, which is four inharmonic partials over a noise
        // transient — nothing this protocol synthesizes (§6 M43). It was
        // `sweep(TRIANGLE, 180 → 90)`, and the difference between a boop and a
        // thunk is exactly the four things `gg-tools timbre` lists.
        CUE_LOCK => Sound::clip("lock", 0.55),
        // Still a tone, and deliberately: this one is *retuned per event* by
        // `clear_voice` below, and a sample cannot be pitched. The milestone
        // added a second kind of voice rather than replacing the first, and the
        // cue that varies is the one that shows why both are worth having.
        CUE_CLEAR => Sound::sweep(wave::SQUARE, 440.0, 660.0, 90, 0.28),
        CUE_HOLD => Sound::sweep(wave::SINE, 520.0, 780.0, 70, 0.22),
        // A major triad struck at once: three pitches, which is three entities
        // under the old protocol and one clip under this one.
        CUE_LEVEL => Sound::clip("level", 0.5),
        // A minor arpeggio falling a whole register with nothing released, so
        // the ending rings. A sweep cannot: a sweep is one voice moving, and
        // the note it left is gone.
        CUE_OVER => Sound::clip("topout", 0.6),
        // Silent at bootstrap, and that is the music's whole state machine —
        // see `step`, where the component being silent is what "not playing"
        // means. A cue bank spawned already looping would start the theme over
        // a title screen.
        _ => Sound::tone(wave::SILENT, 0.0, 0, 0.0),
    }
}

/// Turn the theme on or off, wherever the music cue is (§6 M43).
///
/// A helper because a loop is a *level* rather than an event, and the edges that
/// move it live in two systems: three belong to the screen (start, pause,
/// resume) and the fourth is a top-out, which changes no screen at all. The
/// component is its own state — a `CLIP_LOOP` here means the theme is playing —
/// so this is a comparison rather than a flag somebody has to remember to clear.
pub fn set_music(world: &mut GameWorld, on: bool) {
    let _ = world.each::<(&Cue, &mut Sound)>(|_, (cue, sound)| {
        if cue.kind != CUE_MUSIC || on == (sound.wave == wave::CLIP_LOOP) {
            return;
        }
        let seq = sound.seq;
        *sound = if on {
            ...
            Sound::tone(wave::SILENT, 0.0, 0, 0.0)
        };
        sound.seq = seq;
        sound.play();
    });
...
/// The theme. Four bars of i–VI–III–VII that loop seamlessly, generated by
/// `gg-tools timbre` so the checked-in `.wav` has a source (§6 M43).
///
/// Quieter than every cue by a wide margin: a bed is under a game rather than
/// in front of it, and the player's own volume scales it *live* — a loop is the
/// one voice that outlives the slider, so this is what the settings screen
/// actually moves.
#[must_use]
pub fn music_voice() -> Sound {
    Sound::music("theme", 0.22)
}
...
pub fn step(world: &mut GameWorld) {
    ...
    // The two music edges no screen change covers: a run ending under the
    // game-over overlay, which stays on the playing screen, and the restart
    // that begins the next one from it (§6 M43).
    if topped_out {
        set_music(world, false);
    } else if began_with.is_some() {
        set_music(world, true);
    }
...
pub fn menu(world: &mut GameWorld) {
    ...
    if let Some((to, keep)) = next {
        ...
        // The theme belongs to the playing screen, so the transition is where
        // it is decided — every other spelling reads the screen a tick late,
        // and one keyed off `Play::over` would play the theme over the title,
