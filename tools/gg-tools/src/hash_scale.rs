...
//!   own per-entity and per-column commitments. Same blake3 granularity, no
//!   sorted walk.
...
fn measure(world: &World, n: u32) -> anyhow::Result<Row> {
    ...
    // instrument can name one. Storage order like the floor, so no `location()`
    // indirection and no sorted walk — but absorbed a *row* at a time with the
    // same per-entity and per-column commitments the contract makes, so blake3
    // sees the same update granularity it does. Landing on the contract means
    // the cost is granularity and the sorted order is innocent; landing on the
    // floor means the order is the whole bill.
    let rowwise = best(|| {
...
fn verdict(per_entity_at: &[(u32, f64)]) {
    ...
    println!(
        "\n  Only one of the three parts above is recoverable *as the same number*.\n  \
         Buffering rows into blocks emits the identical byte stream, so the digest is\n  \
         bit-identical and it is not a Determinism Contract event. The sorted order is\n  \
         the contract itself (§4.2.1 sorts by entity id so the hash is layout-\n  \
         independent). And skipping unchanged chunks — the accelerator `chunked.rs`\n  \
         designs — produces a layout-locked number that its own header says can never\n  \
         be the one a gate reads. The cheapest large win was never designed, because\n  \
         the design went straight past it to chunking."
    );
}
